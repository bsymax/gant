...
- 用户**不再使用 `email` 作关联**；`User` 表改为 **`erp`（工号/ERP）** 唯一。迁移 `20260426120000_user_email_to_erp` 将**已有库**的 `email` 值拷入 `erp`（建议再用 CSV/JSON 重灌为真实工号）。
- `initial-data.json`、四张 CSV 列名、种子与转换脚本**全部**改为 `erp` / `userErp` / `assigneeErps` / `memberErps`。

### 内容

- `config/initial-csv/*.csv` 已换为**新示例**（8 人 + 4 项目 + 样例任务），可在其上直接改。  
- 文档（初始化说明、配置表、CSV 流程、系统文件地图、DEV-PLAN）已同步。  
- `web` 包版本 **0.4.0**。
